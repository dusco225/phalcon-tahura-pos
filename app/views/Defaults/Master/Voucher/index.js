window.defaultUrl = `${baseUrl}/master/voucher/`;
var table;
var rupiahFields = [
    "harga",
  ];
...
$(document).ready(function() {
    ...
    $('#btn-add').click(function() {
        ...
        modal.find("select[name=produk_id]").val('').trigger('change');
        modal.find('input[name=diskon]').val('');
        modal.find('input[name=jumlah]').val('');
        modal.find('input[name=mulai_saat]').val('');
        modal.find('input[name=berakhir_saat]').val('');
    ...
    $('#btn-edit').click(function () {
        ...
        if (selected) {
            ...
            modal.find('input[name=diskon').val(selected.diskon);
            modal.find('input[name=jumlah').val(selected.jumlah);
            modal.find('input[name=mulai_saat').val(selected.mulai_saat);
            modal.find('input[name=berakhir_saat').val(selected.berakhir_saat);
            $("select[name=produk_id]").select2("trigger", "select", { data: { id: selected.produk_id, text : selected.nama_produk} });
            convertRupiah();
            resetErrors();
            modal.modal('show');
        }
